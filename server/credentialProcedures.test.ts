...
function createContext(role: "user" | "admin" | null = null): { ctx: TrpcContext; setCookies: Array<{ name: string; value: string }> } {
...
describe("credential procedures", () => {
  ...
  it("issues a local session when student credentials are valid and rejects an incorrect password", async () => {
    ...
    vi.mocked(db.findLocalCredentialByEmail).mockResolvedValue({ userId: 21, passwordHash, role: "user", name: "Student", email: "student@example.com" });
