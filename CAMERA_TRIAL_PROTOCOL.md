...
The final storage-backed and full-stack integrity audit scenarios passed again. They exercise the configured camera-interruption, face-absence, multiple-face, fullscreen-exit, and focus-loss signals through the real proctoring procedure, persisted event records, threshold submission, review timeline, ownership boundary, administrator review, and alert route. This is **simulated signal evidence**, not an observation that a live participant cheated.

The live trial established browser camera permission, local single-face readiness, and the real attempt/submission/review workflow. A separate live multiple-face observation was deliberately not run because no second independently consenting adult was present. A chat-independent acceptance check is likewise deferred: switching away from the fullscreen assessment to report status in this environment is itself the strict focus-loss signal. These are documented deployment-validation limits, not missing data disguised as a pass. Before high-stakes use, an institution should conduct that final hands-on check with informed participants, approved accommodations, and an administrator available to review the resulting attempt.
