...
var ENV = {
  ...
  publicOrigin: process.env.PROCTORX_PUBLIC_ORIGIN ?? "",
...
async function reserveIdsInTransaction(requests, transaction) {
  ...
  for (const { entity, count } of requests) {
    ...
    allocations[entity] = Array.from({ length: count }, (_, index2) => firstId + index2);
...
async function nextIdsInTransaction(entity, count, transaction) {
...
async function createExam(adminUserId, input) {
  ...
  const id = await db.runTransaction(async (transaction) => {
    ...
    input.questions.forEach((question, index2) => transaction.create(examRef.collection("questions").doc(String(questionIds[index2])), { id: questionIds[index2], examId, ...question, orderIndex: index2, createdAt: now, updatedAt: now }));
    return examId;
  });
...
async function updateExam(adminUserId, examId, input) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const now = Timestamp.now();
    transaction.update(examRef, { title: input.title, description: input.description ?? null, durationSeconds: input.durationSeconds, startsAt: timestamp2(input.startsAt), endsAt: timestamp2(input.endsAt), status: input.status, maxAttempts: input.maxAttempts, shuffleQuestions: asBoolNumber(input.shuffleQuestions), releaseResultsImmediately: asBoolNumber(input.releaseResultsImmediately), proctoringConfig: normalizeProctoringConfig(input.proctoringConfig), updatedAt: now });
...
async function nextIdInTransaction2(entity, transaction) {
  const db = getFirebaseFirestore();
  const counter = db.collection("meta").doc("counters");
...
async function nextId2(entity) {
  const db = getFirebaseFirestore();
  return db.runTransaction((transaction) => nextIdInTransaction2(entity, transaction));
}
async function upsertUser(input) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const id = existingLookup.exists ? Number(existingLookup.data().userId) : await nextIdInTransaction2("users", transaction);
...
async function createLocalStudent(input) {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const id = await nextIdInTransaction2("users", transaction);
    const now = Timestamp2.now();
    const record = {
      id,
      openId: input.openId,
...
async function createFirebaseStudent(input) {
  const db = getFirebaseFirestore();
  const emailLookup = db.collection("userLookup").doc("email").collection("entries").doc(lookupId(input.email));
  const openIdLookup = db.collection("userLookup").doc("openId").collection("entries").doc(lookupId(input.openId));
...
async function createFirebaseStudent2(input) {
  if (!useFirestorePersistence()) throw new Error("Firebase Email/Password accounts require Firebase-backed persistence.");
  return createFirebaseStudent(input);
}
async function getUserByFirebaseUid2(firebaseUid) {
...
var FIREBASE_WEB_API_KEY_ENV = "FIREBASE_WEB_API_KEY";
var IDENTITY_TOOLKIT_URL = "https://identitytoolkit.googleapis.com/v1/accounts";
var FirebaseAuthConfigurationError = class extends Error {
};
var FirebaseAuthCredentialsError = class extends Error {
};
var FirebaseAuthRequestError = class extends Error {
  constructor(code) {
...
function approvedContinueUrl(requestedOrigin) {
...
async function requestIdentityToolkitWithAdminAuth(action, payload) {
...
async function requestIdentityToolkit(action, payload) {
...
function canFallbackToFirebaseDefaultActionHandler(error) {
...
async function deleteFirebaseEmailPasswordUser(uid) {
  await getFirebaseAuth().deleteUser(uid);
}
async function authenticateFirebaseEmailPassword(input) {
  const session = await requestIdentityToolkit("signInWithPassword", { email: input.email, password: input.password, returnSecureToken: true });
...
async function sendFirebaseVerificationEmail(idToken, origin) {
  try {
...
async function resendFirebaseVerificationEmail(email, origin) {
  let user;
  try {
    user = await getFirebaseAuth().getUserByEmail(email);
  } catch (error) {
    const code = typeof error === "object" && error && "code" in error ? String(error.code) : "";
    if (code.includes("user-not-found")) return { mode: "sent" };
    throw error;
  }
  if (user.emailVerified) return { mode: "sent" };
  const customToken = await getFirebaseAuth().createCustomToken(user.uid);
  const session = await requestIdentityToolkit("signInWithCustomToken", { token: customToken, returnSecureToken: true });
...
async function sendFirebasePasswordResetEmail(email, origin) {
  try {
    try {
...
function firebaseAuthFailure(error) {
  if (error instanceof FirebaseAuthCredentialsError) return new TRPCError3({ code: "UNAUTHORIZED", message: error.message });
  return error;
}
function firebaseEmailActionFailureCode(error) {
...
var proctorxRouter = router({
  credentials: router({
    signUp: publicProcedure.input(z2.object({ fullName: z2.string().trim().min(2).max(255), email: z2.string().trim().email().max(320), password: z2.string().min(10).max(128), collegeName: z2.string().trim().max(255).nullable().optional(), rollNumber: z2.string().trim().max(128).nullable().optional(), origin: z2.string().url() })).mutation(async ({ ctx, input }) => {
      ...
      if (isFirebaseEmailPasswordAuthenticationConfigured()) {
        ...
        try {
          ...
          const user2 = await createFirebaseStudent2({ ...input, email, firebaseUid: firebaseUser.uid, openId: `firebase:${firebaseUser.uid}` });
          proctorxUserCreated = true;
          const firebaseSession = await authenticateFirebaseEmailPassword({ email, password: input.password });
          let verificationDelivery2;
          try {
            verificationDelivery2 = await sendFirebaseVerificationEmail(firebaseSession.idToken, input.origin);
          } catch (error) {
            ...
            verificationDelivery2 = { mode: rateLimited ? "retry_later" : "configuration_required" };
          }
          return { id: user2.id, verificationDelivery: verificationDelivery2 };
        } catch (error) {
          if (firebaseUser && !proctorxUserCreated) await deleteFirebaseEmailPasswordUser(firebaseUser.uid).catch(() => void 0);
      ...
      if (isFirebaseEmailPasswordAuthenticationConfigured()) {
        try {
          ...
          if (!firebaseSession.emailVerified) {
            ...
            throw new TRPCError3({ code: "FORBIDDEN", message: "Verify your email before signing in. A new verification email has been sent." });
          }
          const account = await getUserByFirebaseUid2(firebaseSession.uid);
          if (!account) throw new TRPCError3({ code: "UNAUTHORIZED", message: "This Firebase account is not registered with ProctorX." });
          await markEmailVerified2(account.id);
          await touchUserSignIn2(account.id);
    ...
    requestVerification: publicProcedure.input(z2.object({ email: z2.string().trim().email().max(320), origin: z2.string().url() })).mutation(async ({ input }) => {
      if (isFirebaseEmailPasswordAuthenticationConfigured()) {
        try {
          return { accepted: true, delivery: await resendFirebaseVerificationEmail(input.email.toLowerCase(), input.origin) };
        } catch (error) {
          const rateLimited = isFirebaseEmailActionRateLimited(error);
          console.error(`[Firebase Auth] Verification resend failed with ${firebaseEmailActionFailureCode(error)}.`);
          return { accepted: true, delivery: { mode: rateLimited ? "retry_later" : "configuration_required" } };
        }
      }
    ...
    verifyEmail: publicProcedure.input(z2.object({ token: z2.string().min(32).max(256) })).mutation(async ({ input }) => {
      if (isFirebaseEmailPasswordAuthenticationConfigured()) throw new TRPCError3({ code: "BAD_REQUEST", message: "Use the secure Firebase verification link sent to your email." });
    ...
    requestPasswordReset: publicProcedure.input(z2.object({ email: z2.string().trim().email().max(320), origin: z2.string().url() })).mutation(async ({ input }) => {
      if (isFirebaseEmailPasswordAuthenticationConfigured()) {
        try {
          return { accepted: true, delivery: await sendFirebasePasswordResetEmail(input.email.toLowerCase(), input.origin) };
        } catch (error) {
          const rateLimited = isFirebaseEmailActionRateLimited(error);
          console.error(`[Firebase Auth] Password reset email action failed with ${firebaseEmailActionFailureCode(error)}.`);
          return { accepted: true, delivery: { mode: rateLimited ? "retry_later" : "sent" } };
        }
      }
      const account = await findLocalAccountByEmail2(input.email.toLowerCase());
    ...
    resetPassword: publicProcedure.input(z2.object({ token: z2.string().min(32).max(256), password: z2.string().min(10).max(128) })).mutation(async ({ ctx, input }) => {
      if (isFirebaseEmailPasswordAuthenticationConfigured()) throw new TRPCError3({ code: "BAD_REQUEST", message: "Use the secure Firebase password-reset link sent to your email." });
...
// server/vercelEntry.ts
var vercelEntry_default = serverless(createServerApp());
export {
